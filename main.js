...
(function (doc) {
  ...
  const enableBtnShowModal = () => {
    ...
    myDialog.querySelector(".selectedOption").innerHTML = found
      ? `Selected option: <b>${found}</b>`
      : "No option selected";
  };

  const showModal = () => {
    doc.getElementById("myDialog").showModal();
  };

  const closeDialog = (e) => {
    const dialog = e.currentTarget.parentElement;
    dialog.close();
    // dialog.querySelector(".selectedOption").innerHTML = "";
  ...
  doc.addEventListener("DOMContentLoaded", function () {
    ...
    const btnCloseDialog = myDialog.querySelector("button");
    if (btnCloseDialog) {
